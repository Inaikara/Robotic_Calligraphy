...
h=legend([p(1) p(2) l1 l2],{'Samples belonging to the first component','Samples belonging to the second component','The pricipal axis of the first component','The pricipal axis of the second component'},'Location','northeast');
...
for j=1:numComponent
    ...
    if j==1
        p(1)=quiver(arrowdata(:,1),arrowdata(:,2),arrowOri(:,1),arrowOri(:,2),'Color',color(j,:),'AutoScaleFactor',0.5);
    else
        p(2)=quiver(arrowdata(:,1),arrowdata(:,2),arrowOri(:,1),arrowOri(:,2),'Color',color(j,:),'AutoScaleFactor',0.5);
    end
    % % 椭圆
    % stdev = sqrtm(3.0.*Sigma(:,:,j));
    % t = linspace(-pi, pi, nbDrawingSeg)';
...
h=legend([p(1) p(2) sp ep l1],{'The direction of the first component','The direction of the second component','The starting point of trajectory','The ending point of trajectory','Trajectory generated by GMR'},'Location','northeast');
set(h,'FontName','Times New Roman','FontSize',12)
axis off
axis equal
hold off
exportgraphics(p1,'.\Figure\fig3a.png','Resolution',600)
exportgraphics(p2,'.\Figure\fig3b.png','Resolution',600)
